...
pooler.load_state_dict(torch.load(os.path.join(last_shard, "pooler.pt"), map_location="cpu"))
pooler.eval()
...
norm.load_state_dict(torch.load(os.path.join(last_shard, "norm.pt"), map_location="cpu"))
norm.eval()
...
def get_result():
    ...
    # Let pooler handle CLS selection
    with torch.no_grad():
        pooled_output = pooler(hs)
